...
for method in methodQueue:
    ...
    if programMode == 'filter':
        ...
        totalVectors = 0
        zmapCommand = """zmap -i eth0 -p {0} -b blocklist.txt -f "saddr udp_pkt_size" --output-filter='sport={0}' -s {1} -M udp --probe-args=hex:{2} -o /dev/null -c 0""".format(methodPort, listeningPort, methodPayloadHex)
